...
- Export "Plan de table (PDF)" entièrement redessiné en vectoriel (cercles/rectangles/sièges/noms positionnés comme à l'écran, via la même géométrie que `TableCanvas`) au lieu d'une liste de noms. Le PDF commence par une page paysage "Plan d'ensemble" (`drawFullPlan`) montrant toutes les tables à leurs positions réelles les unes par rapport aux autres (mêmes coordonnées `table.x`/`table.y` que la "vue d'ensemble" à l'écran, donc si l'utilisateur a glissé une table, ça se reflète dans l'export), suivie d'une page portrait détaillée par table (`drawTableDiagram`). `drawTableAtScale` factorise le dessin d'une table entre les deux modes (échelle propre à la table vs échelle partagée du plan d'ensemble).
...
**🐛 Bug résolu (rapporté 2026-07-16 par test sur téléphone réel, corrigé le même jour)** : le glisser-déposer d'une table sur `TableCanvas` était peu ergonomique sur mobile — un geste de glissement ne déplaçait la table que d'environ 5mm, et il était impossible de la faire descendre. Cause réelle : le viewBox du plan se recalculait en direct pendant le glisser (`minX/minY/maxX/maxY` dépendaient de la position *live* de la table en cours de déplacement, pour ne jamais couper un nom qui dépasserait) — hors la conversion écran→SVG (`getScreenCTM()`) dépend du viewBox actif, donc elle changeait à chaque `pointermove`, créant une boucle de rétroaction qui amortissait le geste (pire en descendant, la direction qui fait le plus grandir le viewBox). Corrigé en séparant deux géométries dans `TableCanvas.jsx` : `tableRenders` (rendu, suit la position live) vs `boundsTableRenders` (uniquement pour le viewBox, basé sur les positions déjà committées dans `tables`, jamais sur `dragTable`) — le viewBox reste stable pendant tout le geste et ne se recale qu'une fois le glisser terminé. Renforcé aussi `touch-action: none` sur `.svg-scroll` (le scroll horizontal du conteneur ne sert plus quasiment à rien depuis que le SVG est responsive, autant éviter qu'il ne dispute le geste). Validé par une mesure précise (pas juste "ça bouge") : ratio déplacement réel / déplacement attendu = 1.00 exactement sur les deux axes, y compris en descente.

**Leçon sur les tests mobiles** : les tests Playwright avec simulation souris (`page.mouse.move/down/move/up`, même en profil `devices['iPhone 13']`) avaient laissé passer ce bug car un simple test "avant/après" avec un seul grand mouvement ne révèle pas un amortissement progressif — il faut mesurer le déplacement réel avec plusieurs petits pas (comme un vrai doigt) et comparer au déplacement attendu selon l'échelle *effective au moment du geste*, pas l'échelle après coup.
