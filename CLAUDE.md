...
**🐛 Bug ouvert, non résolu (rapporté 2026-07-16 par test sur téléphone réel) :** le glisser-déposer d'une table sur `TableCanvas` est peu ergonomique sur mobile — un geste de glissement ne déplace la table que d'environ 5mm, et il est impossible de la faire descendre. Les tests automatisés Playwright (simulation souris via `page.mouse.move/down/move/up`, même en profil `devices['iPhone 13']`) passaient et ne l'ont PAS détecté — ne pas se fier à ces tests pour valider une interaction tactile de ce type, un test sur vrai appareil (ou une simulation `PointerEvent`/`TouchEvent` plus fidèle, à petits incréments) est nécessaire. Piste à vérifier en premier : `handleTablePointerDown`/`toSvgPoint` dans `TableCanvas.jsx` (conversion écran→SVG via `getScreenCTM()`) combiné au redimensionnement responsive du SVG ajouté pour le mode mobile (`max-width:100%; height:auto` en CSS) — à investiguer et corriger avant de considérer le canvas prêt pour les vidéos de lancement.

Prochaine étape : corriger le bug de glisser-déposer mobile ci-dessus en priorité (l'utilisateur va filmer des vidéos montrant l'outil), puis J10 (contenus de lancement), une fois les mentions légales complétées et Search Console vérifié côté utilisateur.
